"""Packaging-friendly GUI entrypoint for the Hospital C desktop client."""
...
from FedLite_Project.Hospital_C.communication.ltx_transfer import (  # noqa: E402
...
DEFAULT_RUNTIME_PATHS = resolve_hospital_runtime_paths("Hospital_C", CURRENT_FILE)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch the FedLiteCare Hospital C research node client."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_RUNTIME_PATHS.config_path,
        help="Optional Hospital C node config path.",
    )
    parser.add_argument(
        "--transfer-config",
        type=Path,
        default=DEFAULT_RUNTIME_PATHS.transfer_config_path,
        help="Optional Hospital C LTX node transfer config path.",
    )
